...
if __name__ == "__main__":
    ...
    results_folder = "logs/poseGoalStateBounded-{}".format(t_stamp())
    if not os.path.exists(results_folder):
        os.makedirs(results_folder)

    min_bounds = torch.tensor([0.5, 0.0, 0.7], device=torch.device("cuda"))
    max_bounds = torch.tensor([0.725, 0.25, 1.0], device=torch.device("cuda"))
