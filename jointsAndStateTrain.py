...
def batch_run(in_batch, model, loss_fn, optimizer=None):
    ...
    prediction = model(current_pose, goal_pose)
    loss = loss_fn(prediction, target_pose)
...
if __name__ == "__main__":
    ...
    train_set, train_loader = load_pose_state_demos(
        image_demo_paths(exp_config["demo_folder"], im_params["file_glob"], from_demo=0, to_demo=95),
    ...
    val_set, validation_loader = load_pose_state_demos(
        image_demo_paths(exp_config["demo_folder"], im_params["file_glob"], from_demo=95),
        exp_config["batch_size"],
        "l_wrist_roll_link",
        "r_wrist_roll_link",
        False,
    ...
    loss_criterion = L1L2Cost(1.0, 1e-2)

    results_folder = "logs/poseGoalState-{}".format(t_stamp())
