...
basis_fs = 30
dt = 0.01
model = DMPNN(in_dim, 2048, pose_hists.shape[2], basis_fs)
...
constraint = constraints.EventuallyReach(2, 1e-2)
...
for epoch in range(300):
    ...
    train_losses = batch_learn(train_loader, loss_fn, None, optimizer)
    avg_train_loss = np.mean(train_losses, 0, keepdims=True)

    # Validation Loop
    # val_losses = batch_learn(val_loader, loss_fn, constraint, None)
    val_losses = batch_learn(val_loader, loss_fn, None, None)
